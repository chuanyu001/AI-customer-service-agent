...
覆盖**行车记录仪、WiFi、流量、加油**四大业务领域的 AI 智能客服 Agent 系统。当前主链路基于 FastAPI + 强规则安全拦截 + 大模型意图理解/问题改写 + 本地向量检索；LangGraph 节点已保留并复用规则服务，但暂未接管 `/chat/message` 主入口。

## ✨ 核心特性

- **本地向量语义检索**：BAAI/bge-small-zh-v1.5 模型，四业务172条知识向量化，启动加载内存后余弦召回
- **大模型意图理解**：豆包优先识别真实意图、抽取 VIN/品牌等槽位，并结合当前会话上下文改写问题
- **当前会话记忆**：`conversation.metadata.memory` 保存当前 session 摘要、槽位和上一轮意图，不做跨会话长期记忆
- **强规则安全拦截**：`keyword_rule` 启动加载到内存，转人工/高风险/超权限/问候语先处理；大模型失败时规则降级
...
| 大模型 | 豆包优先做意图理解/问题改写/槽位抽取；低置信候选重排和润色可选 |
...
小程序3入口 → H5统一页面 → FastAPI后端 → chat.py主链路
                    ...
                    ┌─────────────────────┼──────────────────────┐
                    ▼                     ▼                      ▼
              豆包大模型             MySQL数据库            转人工工单
       (意图理解/上下文改写)   (知识库/向量/会话/记忆)      (待接七鱼)
```
...
用户消息
  ...
  ├─ ④ 大模型理解 (最近6条消息+memory+pending→意图/槽位/改写问题)
  ├─ ⑤ 知识检索 (原问题精确→改写问题精确→改写问题向量→低置信LLM候选重排)
...
LLM_UNDERSTANDING_TIMEOUT=8
LLM_UNDERSTANDING_HISTORY_LIMIT=6
```

> 当前主路径优先调用大模型做意图理解和问题改写，但不允许大模型自由生成知识库外答案。豆包失败或超时时自动降级到规则识别 + 向量检索。
...
- ✅ 大模型意图理解与问题改写（豆包优先，失败后规则降级）
- ✅ 当前会话记忆（最近消息 + memory slots + pending 上下文传给 LLM）
...
- ✅ 火山方舟大模型接入
- ✅ H5前端页面
- ✅ 多业务路由/LLM理解/意图/转人工/品牌/检索 500 条回归通过
